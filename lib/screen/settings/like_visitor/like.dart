...
class Like_Screen extends StatefulWidget {
  @override
  _Like_ScreenState createState() => _Like_ScreenState();
}
...
class _Like_ScreenState extends State<Like_Screen> {
  ...
  Widget getlistchatItem(User nDataList) {
    return Container(
      ...
      child: ListTile(
        onTap: (){
         Navigator.pushNamed(context, Constant.ChatUserDetail, arguments: {"user_id": nDataList.user_id, "type": "3"});
        },
        leading: CircleAvatar(
          backgroundColor:Custom_color.WhiteColor,
          radius: 28,
          backgroundImage: NetworkImage(nDataList.profile_img,scale: 1.0),
        ),
  ...
  get _appbar {
    return PreferredSize(
      ...
      child: AppBar(
        title: CustomWigdet.TextView(
            text: AppLocalizations.of(context).translate("liked").toUpperCase(),
            textAlign: TextAlign.center,
  ...
  Future<List<User>> _ItemList() async {
    try {
      ...
      String url = WebServices.GetUserLike +
